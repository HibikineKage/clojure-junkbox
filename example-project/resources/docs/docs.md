...
The `home-routes` handler in the `example-project.routes.home` namespace
...
The `home-page` function will in turn call the `example-project.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `example-project.handler` namespace:
...
Request middleware functions are located under the `example-project.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `example-project.dev-middleware` namespace found in
